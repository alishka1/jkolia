...
lid_path = (".block--block.text--grey.text--fs-12>a")

def findLid(driver, theme):
	try:
	...
	try:
		value = driver.find_element_by_css_selector(lid_path)
		result = value.get_attribute('text')
		if result == u'Лид':
			driver.find_element_by_css_selector(lid_path).click()
			print(u'Произведен клик по Лид -                    ОК')
			...
			print(u'Текст ссылки не равен Лид -                 ОШИБКА')
			raise SystemExit(0)
	except:
